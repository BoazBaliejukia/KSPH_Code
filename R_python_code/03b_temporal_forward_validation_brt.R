...
# earlier years, then predicts the held-forward year:
...
find_code_dir <- function() {
  ...
  stop(
    ...
    "From the parent repo run source('KSPH_Code/R_python_code/03b_temporal_forward_validation_brt.R'), ",
    "or setwd() into KSPH_Code and source('R_python_code/03b_temporal_forward_validation_brt.R').",
    call. = FALSE
  )
}
...
TEMPORAL_TARGET_YEARS <- 2022:2025
...
TEMPORAL_EVALUATION_POINT_PREDICTIONS_CSV <- file.path(
  TEMPORAL_EVALUATION_DIR,
  "tfv_training_point_predictions.csv"
)
TEMPORAL_EVALUATION_METRICS_BY_YEAR_CSV <- file.path(
  TEMPORAL_EVALUATION_DIR,
  "tfv_performance_metrics_by_year.csv"
)
TEMPORAL_EVALUATION_METRICS_OVERALL_CSV <- file.path(
  TEMPORAL_EVALUATION_DIR,
  "tfv_performance_metrics_overall.csv"
)
TEMPORAL_EVALUATION_CARET_METRICS_BY_YEAR_CSV <- file.path(
  TEMPORAL_EVALUATION_DIR,
  "tfv_caret_metrics_by_year.csv"
)
TEMPORAL_EVALUATION_CARET_METRICS_OVERALL_CSV <- file.path(
  TEMPORAL_EVALUATION_DIR,
  "tfv_caret_metrics_overall.csv"
)
TEMPORAL_EVALUATION_ROC_CURVE_CSV <- file.path(
  TEMPORAL_EVALUATION_DIR,
  "tfv_roc_curve_overall.csv"
)
TEMPORAL_EVALUATION_PR_CURVE_CSV <- file.path(
  TEMPORAL_EVALUATION_DIR,
  "tfv_pr_curve_overall.csv"
)
TEMPORAL_EVALUATION_ROC_PNG <- file.path(
  TEMPORAL_EVALUATION_DIR,
  "tfv_roc_curve_overall.png"
)
TEMPORAL_EVALUATION_PR_PNG <- file.path(
  TEMPORAL_EVALUATION_DIR,
...
OUTPUT_DIR <- TEMPORAL_OUTPUT_BASE_DIR
PREDICTION_TABLE_DIR <- file.path(OUTPUT_DIR, "pred", "tables")
ANNUAL_SUMMARY_RASTER_DIR <- file.path(OUTPUT_DIR, "pred", "annual_rasters")
ANNUAL_SUMMARY_PLOT_DIR <- file.path(OUTPUT_DIR, "pred", "annual_plots")
ROR_ESTIMATE_DIR <- file.path(OUTPUT_DIR, "pred", "ror")
ROR_ESTIMATE_PLOT_DIR <- file.path(ROR_ESTIMATE_DIR, "plots")
ROR_CHANGE_DIR <- file.path(OUTPUT_DIR, "pred", "ror_1yr")
ROR_CHANGE_PLOT_DIR <- file.path(ROR_CHANGE_DIR, "plots")
RAW_ODDS_CHANGE_DIR <- file.path(OUTPUT_DIR, "pred", "odds_1yr")
RAW_ODDS_CHANGE_PLOT_DIR <- file.path(RAW_ODDS_CHANGE_DIR, "plots")
PIXEL_RELATIVE_PREDICTION_DIR <- file.path(OUTPUT_DIR, "pred", "pixel_rel")
PIXEL_RELATIVE_PREDICTION_PLOT_DIR <- file.path(PIXEL_RELATIVE_PREDICTION_DIR, "plots")
MEAN_DERIVED_FIGURE_DIR <- file.path(OUTPUT_DIR, "pred", "mean_top1_figures")
MODEL_DIAGNOSTIC_DIR <- file.path(OUTPUT_DIR, "diag")
MODEL_DIAGNOSTIC_PLOT_DIR <- file.path(MODEL_DIAGNOSTIC_DIR, "plots")
SHAP_DIR <- file.path(OUTPUT_DIR, "pred", "shap")
...
MODEL_PREDICTION_TABLE_CSV <- file.path(
  ...
  "prediction_grid_model_predictions_temporal_target.csv"
)
ANNUAL_SUMMARY_PREDICTION_CSV <- file.path(
  PREDICTION_TABLE_DIR,
  "prediction_grid_prediction_summaries_temporal_target.csv"
)
SHAP_MEAN_TABLE_CSV <- file.path(
  SHAP_TABLE_DIR,
  "prediction_grid_mean_shap_temporal_target.csv"
)
SHAP_COMPLETE_TABLE_CSV <- file.path(
  SHAP_TABLE_DIR,
  "prediction_grid_predictions_with_mean_shap_temporal_target.csv"
)
SHAP_CHANGE_TABLE_CSV <- file.path(
  SHAP_TABLE_DIR,
  "prediction_grid_mean_shap_changes_temporal_target.csv"
)
...
RUN_TEMPORAL_PNG_EXPORTS <- TRUE
RUN_TEMPORAL_MODEL_DIAGNOSTICS <- FALSE
OVERWRITE_RELATIVE_IMPORTANCE_OUTPUTS <- TRUE
OVERWRITE_MARGINAL_EFFECT_OUTPUTS <- TRUE
RUN_SHAP_CALCULATIONS <- FALSE
...
temporal_training_end_year <- function(target_year) {
  as.integer(target_year) - 1L
}
...
temporal_run_name <- function(target_year) {
  ...
  sprintf(
    "tr%s_%s_p%s",
    TEMPORAL_START_YEAR,
    temporal_training_end_year(target_year),
    target_year
  )
}

temporal_comparison_map_name <- function(years) {
  years <- sort(unique(as.integer(years)))
  sprintf("maps_%s_%s", min(years), max(years))
}

temporal_comparison_map_dir <- function(years) {
  file.path(TEMPORAL_OUTPUT_BASE_DIR, temporal_comparison_map_name(years))
}

set_temporal_run_paths <- function(target_year) {
  target_year <- as.integer(target_year)
  training_end_year <- temporal_training_end_year(target_year)
  if (training_end_year < TEMPORAL_START_YEAR) {
    stop(
      "Target year ",
      target_year,
      " has no prior training years with TEMPORAL_START_YEAR = ",
      TEMPORAL_START_YEAR,
      ".",
      call. = FALSE
    )
  ...
  MODEL_DIR <<- file.path(TEMPORAL_MODEL_BASE_DIR, run_name)
  OUTPUT_DIR <<- file.path(TEMPORAL_OUTPUT_BASE_DIR, run_name)
  PREDICTION_TABLE_DIR <<- file.path(OUTPUT_DIR, "pred", "tables")
  ANNUAL_SUMMARY_RASTER_DIR <<- file.path(OUTPUT_DIR, "pred", "annual_rasters")
  ANNUAL_SUMMARY_PLOT_DIR <<- file.path(OUTPUT_DIR, "pred", "annual_plots")
  ROR_ESTIMATE_DIR <<- file.path(OUTPUT_DIR, "pred", "ror")
  ...
  MODEL_PREDICTION_TABLE_CSV <<- file.path(
    PREDICTION_TABLE_DIR,
    sprintf("prediction_grid_model_predictions_%s.csv", target_year)
  )
  ANNUAL_SUMMARY_PREDICTION_CSV <<- file.path(
    PREDICTION_TABLE_DIR,
    sprintf("prediction_grid_prediction_summaries_%s.csv", target_year)
  )
  SHAP_MEAN_TABLE_CSV <<- file.path(
    SHAP_TABLE_DIR,
    sprintf("prediction_grid_mean_shap_%s.csv", target_year)
  )
  SHAP_COMPLETE_TABLE_CSV <<- file.path(
    SHAP_TABLE_DIR,
    sprintf("prediction_grid_predictions_with_mean_shap_%s.csv", target_year)
  )
  SHAP_CHANGE_TABLE_CSV <<- file.path(
    SHAP_TABLE_DIR,
    sprintf("prediction_grid_mean_shap_changes_%s.csv", target_year)
  )
...
set_temporal_comparison_paths <- function(years) {
  years <- sort(unique(as.integer(years)))
  if (length(years) == 0) {
    stop("No years were supplied for temporal comparison map outputs.", call. = FALSE)
  }

  comparison_name <- temporal_comparison_map_name(years)
  PREDICTION_YEARS <<- years

  MODEL_DIR <<- file.path(TEMPORAL_MODEL_BASE_DIR, comparison_name)
  OUTPUT_DIR <<- temporal_comparison_map_dir(years)
  PREDICTION_TABLE_DIR <<- file.path(OUTPUT_DIR, "pred", "tables")
  ANNUAL_SUMMARY_RASTER_DIR <<- file.path(OUTPUT_DIR, "pred", "annual_rasters")
  ANNUAL_SUMMARY_PLOT_DIR <<- file.path(OUTPUT_DIR, "pred", "annual_plots")
  ROR_ESTIMATE_DIR <<- file.path(OUTPUT_DIR, "pred", "ror")
  ...
  MODEL_PREDICTION_TABLE_CSV <<- file.path(
    PREDICTION_TABLE_DIR,
    sprintf("prediction_grid_model_predictions_%s.csv", comparison_name)
  )
  ANNUAL_SUMMARY_PREDICTION_CSV <<- file.path(
    PREDICTION_TABLE_DIR,
    sprintf("prediction_grid_prediction_summaries_%s.csv", comparison_name)
  )
  SHAP_MEAN_TABLE_CSV <<- file.path(
    SHAP_TABLE_DIR,
    sprintf("prediction_grid_mean_shap_%s.csv", comparison_name)
  )
  SHAP_COMPLETE_TABLE_CSV <<- file.path(
    SHAP_TABLE_DIR,
    sprintf("prediction_grid_predictions_with_mean_shap_%s.csv", comparison_name)
  )
  SHAP_CHANGE_TABLE_CSV <<- file.path(
    SHAP_TABLE_DIR,
    sprintf("prediction_grid_mean_shap_changes_%s.csv", comparison_name)
  )
...
make_temporal_output_dirs <- function() {
  ...
  if (RUN_TEMPORAL_MODEL_DIAGNOSTICS) {
...
write_prediction_shap_outputs <- function(prediction_grid, model_prediction_table, predictor_names, model_list, dsl) {
  ...
  if (length(unique(prediction_with_shap$year)) < 2) {
    message("Only one prediction year is present; skipping year-to-year SHAP change table.")
...
check_temporal_evaluation_settings <- function() {
...
classify_temporal_evaluation_predictions <- function(point_predictions) {
...
plot_roc_curve <- function(curve_table, auc, output_png) {
  if (!RUN_TEMPORAL_PERFORMANCE_FIGURES) {
...
plot_pr_curve <- function(curve_table, auc, output_png) {
  ...
  p <- ggplot2::ggplot(curve_table, ggplot2::aes(x = recall, y = precision)) +
    ...
    ggplot2::labs(
      title = sprintf("Precision-Recall Curve (PR-AUC = %.3f)", auc),
      x = "Recall",
      y = "Precision"
    ) +
    ggplot2::theme_bw(base_size = 14) +
...
write_performance_curve_outputs <- function(point_predictions) {
  if (!RUN_TEMPORAL_PERFORMANCE_FIGURES) {
    message("RUN_TEMPORAL_PERFORMANCE_FIGURES is FALSE; skipping ROC/PR curve outputs.")
    return(invisible(NULL))
  }
  ...
  make_parent_dir(TEMPORAL_EVALUATION_ROC_CURVE_CSV)
  utils::write.csv(roc_result$curve, TEMPORAL_EVALUATION_ROC_CURVE_CSV, row.names = FALSE)
  utils::write.csv(pr_result$curve, TEMPORAL_EVALUATION_PR_CURVE_CSV, row.names = FALSE)
  plot_roc_curve(roc_result$curve, roc_result$auc, TEMPORAL_EVALUATION_ROC_PNG)
  plot_pr_curve(pr_result$curve, pr_result$auc, TEMPORAL_EVALUATION_PR_PNG)

  message("Saved ROC curve table: ", TEMPORAL_EVALUATION_ROC_CURVE_CSV)
  message("Saved ROC curve figure: ", TEMPORAL_EVALUATION_ROC_PNG)
  message("Saved PR curve table: ", TEMPORAL_EVALUATION_PR_CURVE_CSV)
  message("Saved PR curve figure: ", TEMPORAL_EVALUATION_PR_PNG)
...
write_caret_performance_outputs <- function(point_predictions) {
  ...
  print_caret_confusion_matrix(point_predictions, "overall held-forward years")
  for (year in sort(unique(point_predictions$year))) {
    print_caret_confusion_matrix(
      point_predictions[point_predictions$year == year, , drop = FALSE],
      paste0("held-forward year ", year)
    )
  }

  utils::write.csv(by_year, TEMPORAL_EVALUATION_CARET_METRICS_BY_YEAR_CSV, row.names = FALSE)
  utils::write.csv(overall, TEMPORAL_EVALUATION_CARET_METRICS_OVERALL_CSV, row.names = FALSE)
  message("Saved caret metrics by year: ", TEMPORAL_EVALUATION_CARET_METRICS_BY_YEAR_CSV)
  message("Saved caret metrics overall: ", TEMPORAL_EVALUATION_CARET_METRICS_OVERALL_CSV)
...
extract_temporal_point_predictions <- function(training_df, evaluation_years) {
  check_temporal_evaluation_settings()

  point_predictions <- list()
  for (year in evaluation_years) {
    year <- as.integer(year)
    run_info <- set_temporal_run_paths(year)
    ...
    if (!file.exists(raster_path)) {
      stop(
        ...
        ": ",
        raster_path,
        "\nRun the temporal prediction for this year first, or remove it from EVALUATION_YEARS.",
        call. = FALSE
      )
    }
    ...
    out$temporal_run_name <- run_info$run_name
    out$training_start_year <- TEMPORAL_START_YEAR
    out$training_end_year <- run_info$training_end_year
    ...
    message(
      ...
      " training-point predictions for ",
      year,
      " from ",
  ...
  if (length(point_predictions) == 0) {
    stop("No temporal evaluation point predictions were created.", call. = FALSE)
  }

  point_predictions <- do.call(rbind, point_predictions)
  row.names(point_predictions) <- NULL
  classify_temporal_evaluation_predictions(point_predictions)
}

summarize_temporal_performance <- function(point_predictions) {
  by_year <- do.call(
    rbind,
    lapply(sort(unique(point_predictions$year)), function(year) {
...
write_temporal_performance_outputs <- function(training_df, evaluation_years) {
  if (!RUN_TEMPORAL_PERFORMANCE_EVALUATION) {
    message("RUN_TEMPORAL_PERFORMANCE_EVALUATION is FALSE; skipping temporal performance evaluation.")
    return(invisible(NULL))
  }

  make_dir(TEMPORAL_EVALUATION_DIR)
  point_predictions <- extract_temporal_point_predictions(training_df, evaluation_years)
  metrics <- summarize_temporal_performance(point_predictions)
  ...
  utils::write.csv(point_predictions, TEMPORAL_EVALUATION_POINT_PREDICTIONS_CSV, row.names = FALSE)
  utils::write.csv(metrics$by_year, TEMPORAL_EVALUATION_METRICS_BY_YEAR_CSV, row.names = FALSE)
  utils::write.csv(metrics$overall, TEMPORAL_EVALUATION_METRICS_OVERALL_CSV, row.names = FALSE)

  message("Saved temporal evaluation point predictions: ", TEMPORAL_EVALUATION_POINT_PREDICTIONS_CSV)
  message("Saved temporal evaluation metrics by year: ", TEMPORAL_EVALUATION_METRICS_BY_YEAR_CSV)
  message("Saved temporal evaluation metrics overall: ", TEMPORAL_EVALUATION_METRICS_OVERALL_CSV)
...
plot_annual_summary_rasters <- function(raster_stack, year) {
  if (!RUN_TEMPORAL_PNG_EXPORTS) {
    message("RUN_TEMPORAL_PNG_EXPORTS is FALSE; skipping annual summary PNG plots.")
    return(invisible(NULL))
  }
...
plot_mean_value_and_top_percentile_figure <- function(raster_stack, output_png, overwrite, title) {
  if (!RUN_TEMPORAL_PNG_EXPORTS) {
    message("RUN_TEMPORAL_PNG_EXPORTS is FALSE; skipping mean/top-1% PNG plot for ", title, ".")
    return(invisible(mean_derived_figure_path(output_png)))
  }
...
plot_multilayer_raster_stack <- function(raster_stack, output_png, overwrite, title) {
  if (!RUN_TEMPORAL_PNG_EXPORTS) {
    message("RUN_TEMPORAL_PNG_EXPORTS is FALSE; skipping PNG plots for ", title, ".")
    return(invisible(output_png))
  }
...
build_pixel_relative_prediction_outputs <- function(years) {
  ...
  for (year in years) {
    ...
    tryCatch(
      ...
      error = function(e) {
        warning(
          "Skipping pixel-relative facet PNG for ",
          year,
          " after raster write succeeded: ",
          conditionMessage(e)
        )
      }
    )
    tryCatch(
      ...
      error = function(e) {
        warning(
          "Skipping pixel-relative mean/top-1% PNG for ",
          year,
          " after raster write succeeded: ",
          conditionMessage(e)
        )
...
temporal_run_annual_summary_raster_file <- function(year) {
  file.path(
...
copy_temporal_annual_summary_raster <- function(year) {
  source_raster <- temporal_run_annual_summary_raster_file(year)
  if (!file.exists(source_raster)) {
    stop(
      "Could not find held-forward annual summary raster for ",
      year,
      ": ",
      source_raster,
      "\nRun the temporal forward-validation model for this year first.",
      call. = FALSE
    )
  }

  target_raster <- annual_summary_raster_path(year)
  if (!file.exists(target_raster) || OVERWRITE_ANNUAL_SUMMARY_RASTERS) {
    raster_stack <- terra::rast(source_raster)
    terra::writeRaster(
      raster_stack,
      target_raster,
    ...
    message("Copied held-forward annual summary raster: ", target_raster)
  } else {
    message("Comparison annual summary raster exists; loading cached file: ", target_raster)
    raster_stack <- terra::rast(target_raster)
  }

  plot_annual_summary_rasters(raster_stack, year)
  invisible(raster_stack)
}

write_temporal_comparison_map_outputs <- function(years) {
  if (!RUN_TEMPORAL_COMPARISON_MAP_OUTPUTS) {
    message("RUN_TEMPORAL_COMPARISON_MAP_OUTPUTS is FALSE; skipping combined temporal map outputs.")
    return(invisible(NULL))
  }
  ...
  missing_rasters <- vapply(years, function(year) !file.exists(temporal_run_annual_summary_raster_file(year)), logical(1))
  if (any(missing_rasters)) {
    missing_years <- years[missing_rasters]
    stop(
      "Cannot build temporal comparison maps because annual summary rasters are missing for: ",
      paste(missing_years, collapse = ", "),
      call. = FALSE
    )
  }

  comparison_info <- set_temporal_comparison_paths(years)
  make_temporal_output_dirs()
  message("Temporal comparison map output directory: ", comparison_info$output_dir)

  invisible(lapply(years, copy_temporal_annual_summary_raster))

  build_ror_outputs(years)
  if (length(years) > 1) {
    build_ror_change_outputs(years)
    build_raw_odds_change_outputs(years)
    build_pixel_relative_prediction_outputs(years)
  } else {
    message("Only one temporal comparison year is present; skipping 1-year change and pixel-relative outputs.")
  }

  invisible(comparison_info)
}
...
#### 1. Read Shared Training And Prediction Datasets ####

require_package("gbm")
require_package("dismo")
require_package("terra")
require_package("sf")
if (RUN_TEMPORAL_PNG_EXPORTS || RUN_TEMPORAL_PERFORMANCE_FIGURES) {
...
message("Temporal model base directory: ", TEMPORAL_MODEL_BASE_DIR)
message("Temporal output base directory: ", TEMPORAL_OUTPUT_BASE_DIR)
message("Temporal target years: ", paste(TEMPORAL_TARGET_YEARS, collapse = ", "))
...
dataset2_all <- filter_training_dataset_by_type(dataset2_all)

predictor_names_all <- identify_predictors(dataset2_all, prediction_grid_all)
dataset2_all <- as_numeric_predictors(dataset2_all, predictor_names_all)
prediction_grid_all <- as_numeric_predictors(prediction_grid_all, predictor_names_all)
...
missing_temporal_prediction_years <- setdiff(TEMPORAL_TARGET_YEARS, sort(unique(prediction_grid_all$year)))
if (length(missing_temporal_prediction_years) > 0) {
  stop(
    "Prediction grid is missing requested temporal target years: ",
    paste(missing_temporal_prediction_years, collapse = ", "),
    call. = FALSE
  )
}

message("Shared training rows available: ", format(nrow(dataset2_all), big.mark = ","))
message("Shared prediction-grid rows available: ", format(nrow(prediction_grid_all), big.mark = ","))
message("Predictors: ", length(predictor_names_all))
message("Full training year range: ", min(dataset2_all$year), "-", max(dataset2_all$year))
...
#### 2. Fit And Predict Forward One Year At A Time ####

temporal_run_records <- vector("list", length(TEMPORAL_TARGET_YEARS))

for (target_index in seq_along(TEMPORAL_TARGET_YEARS)) {
  target_year <- as.integer(TEMPORAL_TARGET_YEARS[target_index])
  run_info <- set_temporal_run_paths(target_year)
  make_temporal_output_dirs()

  message("")
  message("============================================================")
  message("Temporal run: ", run_info$run_name)
  message("Training years: ", TEMPORAL_START_YEAR, "-", run_info$training_end_year)
  message("Prediction year: ", target_year)
  message("Model directory: ", MODEL_DIR)
  message("Output directory: ", OUTPUT_DIR)
  message("============================================================")

  dataset2 <- dataset2_all[
    dataset2_all$year >= TEMPORAL_START_YEAR & dataset2_all$year <= run_info$training_end_year,
    ,
    drop = FALSE
  ]
  prediction_grid <- prediction_grid_all[prediction_grid_all$year == target_year, , drop = FALSE]

  if (nrow(dataset2) == 0) {
    stop("No training rows are available for ", run_info$run_name, ".", call. = FALSE)
  }
  if (nrow(prediction_grid) == 0) {
    stop("No prediction-grid rows are available for target year ", target_year, ".", call. = FALSE)
  }

  check_training_dataset(dataset2, predictor_names_all)
  utils::write.csv(data.frame(predictor = predictor_names_all), PREDICTOR_NAMES_CSV, row.names = FALSE)
  saveRDS(predictor_names_all, PREDICTOR_NAMES_RDS)

  event_count <- sum(dataset2[[OUTCOME_COLUMN]] == EVENT_VALUE)
  control_count <- sum(dataset2[[OUTCOME_COLUMN]] == CONTROL_VALUE)
  message("Training rows retained: ", format(nrow(dataset2), big.mark = ","))
  ...
  message("Creating ", N_DATASETS, " sampled datasets with ", CONTROLS_PER_EVENT, " controls per event...")
  set.seed(RANDOM_SEED + target_year)
  dsl <- vector("list", N_DATASETS)
  for (j in seq_len(N_DATASETS)) {
    dsl[[j]] <- make_sampled_dataset(
      dataset2,
      controls_per_event = CONTROLS_PER_EVENT,
      seed = RANDOM_SEED + target_year + j,
  ...
  for (j in seq_along(dsl)) {
    message("Training BRT model ", j, " of ", length(dsl), " for ", run_info$run_name, "...")
    model_list[[j]] <- fit_brt_model(dsl[[j]], predictor_names_all, model_id = j)
  }
  ...
  if (!file.exists(MODEL_PREDICTION_TABLE_CSV) || OVERWRITE_MODEL_PREDICTION_TABLE) {
    message("Predicting event probability for ", target_year, "...")
    model_prediction_table <- make_model_prediction_table(prediction_grid, predictor_names_all, model_list)
  ...
  if (RUN_TEMPORAL_MODEL_DIAGNOSTICS) {
    importance_summary <- write_relative_importance_outputs(model_list, predictor_names_all)
    write_marginal_effect_outputs(model_list, importance_summary)
  } else {
    message("Skipping variable-importance and marginal-effect diagnostics for this temporal run.")
  }

  if (!file.exists(ANNUAL_SUMMARY_PREDICTION_CSV) || OVERWRITE_ANNUAL_SUMMARIES) {
    message("Calculating min, max, mean, and median prediction columns for ", target_year, "...")
  ...
  message("Rasterizing and plotting annual prediction summaries for ", target_year, "...")
  summary_raster <- prediction_summary_to_raster_stack(annual_summary_predictions)
  output_tif <- annual_summary_raster_path(target_year)
  terra::writeRaster(
    summary_raster,
    output_tif,
    overwrite = OVERWRITE_ANNUAL_SUMMARY_RASTERS,
  ...
  plot_annual_summary_rasters(summary_raster, target_year)

  write_prediction_shap_outputs(
    prediction_grid = prediction_grid,
    model_prediction_table = model_prediction_table,
    predictor_names = predictor_names_all,
    model_list = model_list,
    dsl = dsl
  )

  build_ror_outputs(target_year)

  temporal_run_records[[target_index]] <- data.frame(
    run_name = run_info$run_name,
    training_start_year = TEMPORAL_START_YEAR,
    training_end_year = run_info$training_end_year,
    prediction_year = target_year,
    training_rows = nrow(dataset2),
    event_rows = event_count,
    control_rows = control_count,
    prediction_rows = nrow(prediction_grid),
...
#### 3. Save Temporal Forward-Validation Index ####

temporal_run_index <- do.call(rbind, temporal_run_records)
make_parent_dir(TEMPORAL_RUN_INDEX_CSV)
utils::write.csv(temporal_run_index, TEMPORAL_RUN_INDEX_CSV, row.names = FALSE)
message("Saved temporal run index: ", TEMPORAL_RUN_INDEX_CSV)


#### 4. Evaluate Held-Forward Prediction Performance ####

# This extracts each held-forward annual prediction raster back to the rows of
# dataset2 from the same year, then evaluates discrimination and thresholded
# classification performance. With the default threshold method/scope, the
# cutoff is optimized one time after concatenating all evaluation years to
# maximize sensitivity * PPV. This gives a consistent threshold for yearly
# confusion matrices, though it is still chosen after observing the held-forward
# outcomes.
write_temporal_performance_outputs(dataset2_all, EVALUATION_YEARS)


#### 5. Create Combined Temporal Map Outputs ####
